...
import {
  ...
  Layers, Radio, Activity, Wifi, Cpu, Target, CheckCircle, LayoutDashboard, BarChart2
} from 'lucide-react';
  ...
  return (
    <div style={{ position: 'relative', width: 96, height: 96, flexShrink: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <span style={{ color: '#FAFAFA', fontSize: 18, fontWeight: 900, letterSpacing: '-0.04em', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>
          {Math.round(pct)}<span style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.4)' }}>%</span>
        </span>
      </div>
    </div>
  );
...
function MonthlyUsageCard({ plan, youtubeConnected, commentsUsed, commentsLimit, trialDaysLeft, onConnectYouTube }:
  ...
  const badgeStyle: React.CSSProperties = {
    display: 'inline-flex', alignItems: 'center', gap: 5, borderRadius: 8, padding: '4px 10px',
    ...
    return (
      <div className="db-card" style={{ display: 'flex', flexDirection: 'column' }}>
        <div className="db-card-header" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <div><div className="db-card-title">Monthly Usage</div><div className="db-card-sub">Comment scan quota</div></div>
  ...
  return (
    <div className="db-card">
      <div className="db-card-header" style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div><div className="db-card-title">Monthly Usage</div><div className="db-card-sub">Comment scan quota</div></div>
        {isFree ? (
          <div style={{ ...badgeStyle, background: 'rgba(245,158,11,0.1)', border: '1px solid rgba(245,158,11,0.22)', color: '#F59E0B' }}><Zap size={9} /> Free Trial</div>
        ) : (
          <div style={{ ...badgeStyle, background: 'rgba(52,211,153,0.1)', border: '1px solid rgba(52,211,153,0.22)', color: '#34d399' }}><CheckCircle size={9} /> Pro</div>
        )}
      </div>
      <div className="db-card-body">
        ...
        {!isFree && (
          <div style={{ display: 'flex', gap: 8 }}>
            <Link href="/billing" className="db-btn-ghost" style={{ flex: 1, textAlign: 'center' }}>Manage Plan</Link>
  ...
  return (
    <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
      <div style={{ position: 'relative', width: 140, height: 140 }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ color: '#FAFAFA', fontSize: 28, fontWeight: 900, letterSpacing: '-0.04em', fontVariantNumeric: 'tabular-nums', lineHeight: 1 }}>
            {pct}<span style={{ fontSize: 16, fontWeight: 700, color: 'rgba(255,255,255,0.5)' }}>%</span>
          </span>
...
export default function Dashboard() {
  ...
  if (aiModel) {
    healthRows.push({ key: 'model', icon: Cpu, label: 'AI Model', sub: aiModel, color: '#a78bfa', dot: 'green' });
  }
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Inter', -apple-system, sans-serif; }
        ...
        /* ── BG ── */
        .db-bg { min-height:100vh; background:#090909; position:relative; overflow:hidden; }
        .db-bg::before { content:''; position:fixed; inset:0; z-index:0; pointer-events:none;
          background: radial-gradient(ellipse 60% 50% at -5% 0%, rgba(245,158,11,0.09) 0%, transparent 65%),
                      radial-gradient(ellipse 55% 45% at 105% 100%, rgba(124,58,237,0.07) 0%, transparent 65%); }
        .db-bg::after { content:''; position:fixed; inset:0; z-index:0; pointer-events:none;
          background-image: linear-gradient(rgba(255,255,255,0.018) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.018) 1px, transparent 1px);
          background-size: 40px 40px; }
        .db-noise { position:fixed; inset:0; z-index:0; pointer-events:none; opacity:0.018;
          background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)'/%3E%3C/svg%3E");
          background-size: 160px 160px; }
        .db-vignette { position:fixed; inset:0; z-index:0; pointer-events:none;
          background: radial-gradient(ellipse 100% 100% at 50% 50%, transparent 55%, rgba(0,0,0,0.55) 100%); }

        /* ── SIDEBAR ── */
        .db-sidebar { width:220px; min-width:220px; background:rgba(10,10,13,0.82); backdrop-filter:blur(28px); -webkit-backdrop-filter:blur(28px);
          border-right:1px solid rgba(255,255,255,0.055); display:flex; flex-direction:column;
          position:fixed; height:100vh; left:0; top:0; z-index:40;
          box-shadow: inset -1px 0 0 rgba(255,255,255,0.03), 4px 0 40px rgba(0,0,0,0.4), 4px 0 16px rgba(245,158,11,0.04); }
        .db-sidebar::before { content:''; position:absolute; left:0; top:18%; bottom:18%; width:2px; border-radius:0 2px 2px 0;
        ...
        .db-logo-mark { width:36px; height:36px; border-radius:11px; flex-shrink:0;
        ...
        .db-logo-sub  { color:rgba(255,255,255,0.28); font-size:10px; font-weight:500; margin-top:1px; }

        .db-nav { flex:1; padding:14px 10px; display:flex; flex-direction:column; gap:2px; overflow-y:auto; }
        .db-nav-item { display:flex; align-items:center; gap:10px; padding:9px 13px; border-radius:12px;
          font-size:13px; font-weight:500; text-decoration:none; color:rgba(255,255,255,0.38);
          transition:all 0.22s cubic-bezier(.4,0,.2,1); border:1px solid transparent; position:relative; overflow:hidden; cursor:pointer; }
        .db-nav-item:hover { background:rgba(255,255,255,0.042); color:rgba(255,255,255,0.78); transform:translateY(-1px); box-shadow:0 4px 16px rgba(0,0,0,0.18); }
        .db-nav-item.active { background:rgba(255,170,35,0.08); color:#F5A623; border-color:rgba(255,170,35,0.22);
          box-shadow:0 0 40px rgba(255,170,35,0.12), inset 0 1px 0 rgba(255,255,255,0.04); backdrop-filter:blur(20px); font-weight:600; }
        .db-nav-item.active::before { content:''; position:absolute; left:0; top:50%; transform:translateY(-50%);
          width:3px; height:22px; border-radius:0 3px 3px 0;
          background:linear-gradient(180deg,#F59E0B 0%,#ec4899 55%,#7C3AED 100%);
          box-shadow:0 0 10px rgba(245,158,11,0.6); animation:indicatorPulse 2.5s ease-in-out infinite; }

        .db-nav-live { display:inline-flex; align-items:center; gap:4px; background:rgba(245,158,11,0.1);
          border:1px solid rgba(245,158,11,0.22); border-radius:8px; padding:2px 7px;
        ...
        .db-upgrade { margin:0 10px 10px; background:rgba(245,158,11,0.055); border:1px solid rgba(245,158,11,0.14); border-radius:14px; padding:15px; }
        ...
        /* ── TOPBAR ── */
        .db-topbar { position:sticky; top:0; z-index:30; background:rgba(9,9,9,0.8); backdrop-filter:blur(24px); -webkit-backdrop-filter:blur(24px);
          border-bottom:1px solid rgba(255,255,255,0.048); padding:0 28px; height:58px;
          display:flex; align-items:center; gap:14px;
          box-shadow:0 1px 0 rgba(255,255,255,0.025), 0 4px 32px rgba(0,0,0,0.28); }
        .db-search { flex:1; max-width:440px; background:rgba(255,255,255,0.038); border:1px solid rgba(255,255,255,0.065);
          border-radius:10px; padding:0 14px 0 36px; height:34px; color:#FAFAFA; font-size:13px; outline:none; transition:all 0.2s; }
        ...
        /* ── WELCOME ── */
        .db-welcome { color:rgba(255,255,255,0.45); font-size:15px; font-weight:600; margin-bottom:4px; letter-spacing:0.01em; }
        .db-page-title { font-size:30px; font-weight:900; color:#FAFAFA; letter-spacing:-0.035em; margin-bottom:0; }

        /* ── STAT CARDS ── */
        .db-stats { display:grid; grid-template-columns:repeat(4,1fr); gap:12px; margin-bottom:16px; }
        .db-stat { background:rgba(15,15,19,0.9); border:1px solid rgba(255,255,255,0.055); border-radius:16px; padding:20px;
          transition:all 0.22s cubic-bezier(.4,0,.2,1); display:flex; flex-direction:column; gap:10px; backdrop-filter:blur(12px); }
        ...
        .db-card { background:rgba(15,15,19,0.9); border:1px solid rgba(255,255,255,0.055); border-radius:16px; backdrop-filter:blur(12px); transition:border-color 0.2s; }
        ...
        /* ── BOTTOM NAV ── */
        .db-bottom-nav { display:none; position:fixed; bottom:0; left:0; right:0; z-index:50;
          background:rgba(9,9,11,0.95); border-top:1px solid #27272A; backdrop-filter:blur(20px);
          padding:8px 0 env(safe-area-inset-bottom,8px); }
        .db-bnav-item { display:flex; flex-direction:column; align-items:center; justify-content:center; gap:3px;
          flex:1; padding:6px 4px; text-decoration:none; color:rgba(255,255,255,0.4);
          font-size:10px; font-weight:500; transition:all 0.2s; border:none; background:none; cursor:pointer; }
        .db-bnav-item.active { color:#F59E0B; }
        .db-bnav-item:hover { color:rgba(255,255,255,0.8); }
      ...
      <div className="db-bg" style={{ display:'flex' }}>
          ...
          <nav className="db-nav">
            {NAV_ITEMS.map(item => (
              <Link key={item.href} href={item.href} className={`db-nav-item${item.active ? ' active' : ''}`}>
                ...
                {item.active && (
                  <span className="db-nav-live"><span className="db-nav-live-dot" /> live</span>
                )}
              </Link>
            ))}
          </nav>
        ...
        <div className="db-main">
          ...
          <header className="db-topbar">
            ...
            <div className="db-status-pill db-topbar-status">
              <div className="db-status-dot" /> All systems operational
            </div>
            <div style={{ flex:1 }} />
            {!youtubeConnected && (
          ...
          <div className="db-content">
            ...
            {/* STAT CARDS — full width, no wasted space */}
                ...
                return (
                  <div key={s.label} className="db-stat">
                    ...
                    <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between' }}>
                      ...
                      : hasReal ? (
                        <><div className="db-stat-value">{s.fmtValue}</div><Sparkline color={s.color} up={s.up} /></>
                      ) : (
        ...
        {/* ── BOTTOM NAV — mobile only, same pattern as analytics ── */}
