  ...
  <span style={{ position: 'absolute', top: 6, right: 6, width: 13, height: 13, background: '#7C3AED', borderRadius: '50%', border: '1.5px solid #0a0a0f', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 7.5, color: 'white', fontWeight: 800 }}>{notifCount}</span>
        ...
        {showBottomNav && <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 50, background: 'rgba(9,9,11,0.97)', borderTop: '1px solid rgba(255,255,255,0.055)', display: 'flex', backdropFilter: 'blur(28px)', paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
          {[
            ...
            { href: '/automation', label: 'Automation', icon: <svg width={isDesktopSiteOn?28:12} height={isDesktopSiteOn?28:15} fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg> },
            { href: '/alerts',     label: 'Alerts',     icon: <svg width={isDesktopSiteOn?28:12} height={isDesktopSiteOn?28:15} fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg> },
          ].map(link => {
